...
class TransferService {
     ...
     * 
     * 
68fc6dd29683f20dd51a403f

{
  "medium": "balance",
  "payee_id": "68fc6dbf9683f20dd51a403e",
"amount": 1000,
  ...
  "description": "string"
     ...
     * 
     */
async createTransfer(payer_id, { payee_id, amount, description }) {
  try {
    const body = {
      medium: "balance",
      payee_id: payee_id, // cuenta destino
      amount: amount,
    ...
    const response = await axios.post(
      `http://api.nessieisreal.com/accounts/${payer_id}/transfers?key=b9c71161ea6125345750dcb92f0df27c`,
      body
    );

    return response.data;
  } catch (error) {
    console.error('Error creando la transferencia:', error.response?.data || error.message);
    return null;
  }
}
